...
def benchmark_saliency(**kwargs):
    ...
    attribution = guided_ig.GetMask(
        ...
        max_dist=kwargs["max_l1_distance"]
    )

    fig = plot_image_attribution(kwargs["image"][0, ...], attribution)
    fig.savefig("benchmark_guided_ig_saliency.png")


def benchmark_feature_attributions(**kwargs):
    guided_ig = GuidedIntegratedGradients(model=kwargs["model"])
    attribution = guided_ig.explain(
        x=np.transpose(kwargs["image"].copy(), (0, 3, 1, 2)),
        baselines=np.transpose(kwargs["baselines"].copy(), (0, 3, 1, 2)),
        target_idx=kwargs["target_idx"],
        n_steps=kwargs["n_steps"],
        fraction=kwargs["fraction"],
        max_l1_distance=kwargs["max_l1_distance"]
    )
    attribution = np.transpose(attribution, (0, 2, 3, 1))

    fig = plot_image_attribution(kwargs["image"][0, ...], attribution[0, ...])
    fig.savefig("benchmark_guided_ig_feature_attributions.png")
