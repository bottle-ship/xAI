...
def benchmark_captum(**kwargs):
    saliency = Saliency(forward_func=kwargs["model"])
    attribution = saliency.attribute(
        inputs=torch.from_numpy(np.transpose(kwargs["image"].copy(), (0, 3, 1, 2))).to(dtype=torch.float64),
    ...
    fig.savefig("benchmark_saliency_captum.png")
